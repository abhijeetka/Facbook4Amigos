...
<html>
  ...
  <?php
    if($user_id) {
      ...
      try {
//TechnoHackzs ComputerFrEaKs2592 developernolife tagcloudmarketing happydentindia GadgetsToUse
        $fb_accounts = $facebook->api('/me/accounts');
        $access_token_post = $fb_accounts['data'][0]['access_token'];
        $user_profile = $facebook->api('/TechnoHackzs/','GET');        
        ...
        $currentDate = date('d');
         ...
         $photo = 'C:/Users/Abhi/Downloads/Hackintosh_Wallpaper_v5_by_Jonzy.png.jpg';
           ...
           // echo 'posted date--->'.$postedDate;
            if (strcmp($postedDate, $currentDate)== 0){
                if (array_key_exists('message', $post)) {
                     ...
                     // echo '<pre>message-->'.$post['message'].'</pre>';
                }
